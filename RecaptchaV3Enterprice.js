 ...
 * facil implementacion recaptchaV3 enterprice
*/

class RecaptchaV3Enterprice {
    _keyPublic;
    constructor(element = Element, keyPublic = String) {
        this._keyPublic = keyPublic;
        const script = document.createElement('script');
        script.src = 'https://www.google.com/recaptcha/enterprise.js?render=' + this._keyPublic;
    ...
    validarRV3S(fn, rutaServidor = 'scaptcha.php', accion = 'login') {
        try {
            grecaptcha.enterprise.ready(async () => {
                const token = await grecaptcha.enterprise.execute(this._keyPublic, {
                    action: accion
                });
    ...
    _validarResRecaptcha(res) {
        ...
        let validador = false;
        //console.log(res);
        if (res.riskAnalysis && res.riskAnalysis.score >= 0.1)
            validador = true;
